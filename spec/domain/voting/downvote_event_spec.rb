...
RSpec.describe Voting::DownvoteEvent, type: :model do
  ...
  describe "#call" do
    ...
    it "does not publish if latest vote is already downvote" do
      event_store = instance_double(RailsEventStore::Client)
      reader = double("event_store_reader")
      allow(Rails.configuration).to receive(:event_store).and_return(event_store)
      allow(event_store).to receive(:read).and_return(reader)
      allow(reader).to receive(:stream).with("User$user_456$Event$84").and_return(reader)
      allow(reader).to receive(:to_a).and_return(
        [ Voting::EventDownvoted.strict(data: { event_id: "84", user_id: "user_456" }) ]
      )
    ...
    it "publishes with numeric expected_version when stream already has events" do
      ...
      allow(reader).to receive(:stream).with("User$user_456$Event$84").and_return(reader)
      allow(reader).to receive(:to_a).and_return(
        [ Voting::EventUpvoted.strict(data: { event_id: "84", user_id: "user_456" }) ]
      )
